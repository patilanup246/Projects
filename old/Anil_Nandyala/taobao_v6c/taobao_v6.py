...
def my_form_post():
    ...
    file_output_taobao = open('taobao_log.txt','w',encoding='utf-8')
    file_output_taobaoUnsorted = open('taobao_logUnsorted.txt','w',encoding='utf-8')
    file_output_tmall = open('tmall_log.txt','w',encoding='utf-8')
    file_output_tmallUnsorted = open('tmall_logUnsorted.txt','w',encoding='utf-8')
    file_output_taobao_price_rating = open('file_output_taobao_price_rating.txt','w',encoding='utf-8')
    file_output_tmall_price_rating = open('file_output_tmall_price_rating.txt','w',encoding='utf-8')
    taobao_cumulative_file = open('taobao_cumulative_file.txt','w',encoding='utf-8')
    tmall_cumulative_file = open('tmall_cumulative_file.txt','w',encoding='utf-8')
    taobao_items = []
    tmall_items = []
    ...
    'cookie': "t=868d19632059e35f1ae1169dd0a75779; cna=zfBWEnM3Ok0CAWfPCpJKdqpK; _m_h5_tk=4b55850ca5345a0ae559610a43a9bf1c_1513262281377; _m_h5_tk_enc=e284f6875990868e0440388a8f8436da; mt=ci=-1_0; hng=CN%7Czh-CN%7CCNY%7C156; thw=cn; v=0; cookie2=7ddef3cd32a74abd8d36be3fc0e08c66; _tb_token_=e65711fef4356; isg=AsvLHoPHFaZuZklCZh-qhS3OWm91yN7E7MwlUD3ITIphXOm-zTR1Mgq-EGpJ",
    ...
    for page in range(0,total_pages):
        ...
        response = requests.get('https://s.taobao.com/search?sort=sale-desc&data-key=s&ajax=true&q={}&loc={}&data-value={}&filter=reserve_price[{},{}]'.format(_keywords,_location,str(page*44),str(min_price),str(max_price)),headers=headers_taobao)
        ...
        for item in response.json()['mods']['itemlist']['data']['auctions']:
            ...
            try:
                ...
                if 'tmall' in item_page_link:
                    ...
                    t_prices = []
                    try:
                        response_tmall_price = requests.request("GET", url, headers=headers_tmall_price, params=querystring)
                        print (response_tmall_price.text)
                        response_tmall_price = response_tmall_price.json()
    ...
    for item in taobao_items:
        ...
        taobao_cumulative_file.write('https:'+item['link']+'\t'+ str(item['goodRating'])+'\t'+ str(item['normalRating'])+'\t'+ str(item['badRating'])+'\t'+str(item['price'])+'\t'+str(item['rating_0'])+'\t'+str(item['rating_1'])+'\t'+str(item['rating_2'])+'\t'+str(item['Seller_stars'])+'\t'+str(item['crown_color'])+'\t'+str(item['Score'])+'\n')
...
<br>'''
    ...
    for i in open('tmall_cumulative_file.txt').read().split('\n'):
        ...
        for j in i.split('\t'):
            if i_l == 0:
                output_html+='<td><a href="https://detail.'+j+'">https://detail.'+j+'</a></td>'
