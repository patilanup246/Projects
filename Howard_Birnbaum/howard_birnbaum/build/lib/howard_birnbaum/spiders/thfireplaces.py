...
class thfireplaces(scrapy.Spider):
    name="thfireplaces"
    all_urls = []
    scraped_country = ''
    ...
    def start_requests(self):
        for url in self.all_urls:
            url1 = url.split('$')
            req_url = "http://www.thfireplaces.ca/wp-admin/admin-ajax.php"
            headers = {
                'content-type': "application/x-www-form-urlencoded",
                'cache-control': "no-cache"
            }
            payload = "lat={}&lng={}&radius=500&action=csl_ajax_search".format(str(url1[1]),str(url1[2]))
