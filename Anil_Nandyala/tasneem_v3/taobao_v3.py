...
Example: Guangdong =
%E5%B9%BF%E5%B7%9E%2C%E6%B7%B1%E5%9C%B3%2C%E4%B8%AD%E5%B1%B1%2C%E7%8F%A0%E6%B5%B7%2C%E4%BD%9B%E5%B1%B1%2C%E4%B8%9C%E8%8E%9E%2C%E6%83%A0%E5%B7%9E
...
for page in xrange(total_pages):
    ...
    #price range
    response = requests.get('https://s.taobao.com/search?sort=sale-desc&data-key=s&ajax=true&cd=false&q={}&loc={}&data-value={}&filter=reserve_price%5B1%2C99999%5D'.format(open('keywords.txt','r').read(),open('location.txt','r').read(),str(page*44)))

    #maximum price
	#response = requests.get('https://s.taobao.com/search?sort=sale-desc&data-key=s&ajax=true&cd=false&q={}&loc={}&data-value={}&filter=reserve_price%5B%2C200%5D'.format(open('keywords.txt','r').read(),open('location.txt','r').read(),str(page*44)))

    #standard
    #response = requests.get('https://s.taobao.com/search?sort=sale-desc&data-key=s&ajax=true&cd=false&q={}&loc={}&data-value={}'.format(open('keywords.txt','r').read(),open('location.txt','r').read(),str(page*44)))
