...
def show_entry_fields():
    #print("First Name: %s\nLast Name: %s" % (urllib.quote(e1.get().decode('utf-8').encode('utf-8')).replace('%252C','%2C'), e2.get()))
    
    
    _location = urllib.quote(e1.get().decode('utf-8').encode('utf-8')).replace('%252C','%2C')
    _keywords = e2.get()
    
    total_pages = int(e3.get())
    min_price = e4.get()
    max_price = e5.get()
    ...
    for page in range(0,total_pages):
        ...
        print ('https://s.taobao.com/search?sort=sale-desc&data-key=s&ajax=true&q={}&loc={}&data-value={}&filter=reserve_price[{},{}]'.format(_keywords,_location,str(page*44)))
...
Label(master, text="No of pages").grid(row=2)
Label(master, text="Min Price").grid(row=3)
Label(master, text="Max Price").grid(row=4)
e1 = Entry(master)
e2 = Entry(master)
e3 = Entry(master)
e4 = Entry(master)
...
Button(master, text='Submit', command=show_entry_fields).grid(row=6, column=1, sticky=W, pady=4)
