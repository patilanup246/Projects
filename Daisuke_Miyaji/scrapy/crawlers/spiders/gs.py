...
class GS(scrapy.Spider):
    ...
    def __init__(self,category='',shops_root_url='',via_page_url_regex='',single_shop_url_regex='',single_sleep=0,via_page_sleep=0,*args,**kwargs):

        filename_json = shops_root_url.split('.')[1]+'.json'
        filename_csv = shops_root_url.split('.')[1]+'.csv'
        ...
        try:
            ...
            q = Queue()
            [q.put(i) for i in final_url_array]
            ...
            file_export = open(filename_csv, 'wb')
            ...
            #yagmail.SMTP(self.user_from, self.user_pass).send(self.user_to2, 'Daisuke API', open(filename_csv, 'r').read())
            file_json = open(filename_json, 'w')
