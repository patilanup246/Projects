...
class finra(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        sql_statement = '''INSERT INTO finra_firms ("firmId", "firmName", "secNumber", "isLegacy", "otherNames", "bcScope", "finraRegistered", "districtName", "firmType", "formedState", "firmSize", "formedDate", "fiscalMonthEndCode", "legacyReportStatus", "branchOfficeLocations", "firmAddressDetails", "bdDisclosureFlag", "disclosures", "registrations", "directOwners", "scrapeID") VALUES ('{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}',{})'''.format(
            ...
            str(resp.get('basicInformation',{}).get('firstName','')).encode('ascii', 'ignore').decode('ascii').replace("'","''"),
            str(resp.get('basicInformation',{}).get('lastName','')).encode('ascii', 'ignore').decode('ascii').replace("'","''"),
            str(resp.get('basicInformation',{}).get('nameSuffix','')).encode('ascii', 'ignore').decode('ascii').replace("'","''"),
            str('$SEP$'.join(resp.get('basicInformation',{}).get('otherNames',''))).encode('ascii', 'ignore').decode('ascii').replace("'","''"),
